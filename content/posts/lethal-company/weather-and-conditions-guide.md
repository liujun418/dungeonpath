...
tags: ["Lethal Company", "PC Games", "Weather Guide", "Survival", "Mechanics", "lethal company weather effects guide", "lethal company eclipsed strategy", "lethal company flooded moons survival"]
categories: ["PC Games"]
...
1. **Terminal command:** While on the ship, use the `weather` command at the Terminal. This displays the current weather for every moon in the current rotation.
2. **Orbit view:** When orbiting a moon (after selecting it but before landing), the weather icon appears next to the moon's name in the ship's UI.
...
**The rush strategy:**
1. Land the ship as close to the Main Entrance as possible. Study the landing zone during the descent.
2. All crew members exit the ship simultaneously. Do not linger on the ship ramp — Eyeless Dogs can patrol right up to the ship.
3. Sprint directly to the entrance. Do not stop for exterior scrap — the interior has plenty.
4. Once inside, standard interior play applies. The scrap multiplier makes every item worth more.
5. On exit, use the same rush tactics. Coordinate a simultaneous sprint back to the ship.
...
1. Identify which fire exits are in low-lying areas. Watch for puddles and drainage patterns on the exterior.
2. Mark the high-ground fire exits as primary escape routes. If a fire exit is on elevated terrain, it will remain accessible all day.
3. If the Main Entrance is on low ground, it may become harder to reach as flooding progresses. Do not rely on it as your only exit.
...
For more on team coordination, see our [Lethal Company Co-op Team Roles Guide](/posts/lethal-company/coop-team-roles-guide/). For moon-specific advice under different weather conditions, the [Lethal Company Moon Guide](/posts/lethal-company/moon-guide/) has detailed breakdowns for every location.
